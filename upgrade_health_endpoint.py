...
def patch_health(text: str) -> str:
    ...
    if search_region:
        ...
        if new_region is None:
            print("⚠️ Couldn't locate existing /health in Supersonic block; appending enhanced version inside the block.")
            new_region = region_text.rstrip() + "\n" + NEW_HEALTH_FUNC + "\n"
        return text[:block_start] + new_region + text[block_end:]
    else:
        print("⚠️ Supersonic block markers not found; replacing first /health we can find globally.")
        replaced = _do_replace(text)
        if replaced is None:
            print("⚠️ No /health found at all; appending enhanced version to end of file.")
            return text.rstrip() + "\n\n" + NEW_HEALTH_FUNC + "\n"
        return replaced
...
def main():
    ...
    print("✅ Upgraded /health endpoint.")
