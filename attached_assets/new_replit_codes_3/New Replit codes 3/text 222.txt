#!/usr/bin/env python3
# version_badge_bootstrap.py — Add a dynamic "Latest Version" badge to MkDocs
  ...
  async function main() {
    try {
      ...
      try {
        ...
        // As a last resort, show placeholder
        insertBadge("latest", relPage);
      }
    }
  }

  // Wait for page render to avoid layout flash
...
def patch_mkdocs():
  mk = ROOT / "mkdocs.yml"
  if not mk.exists():
    print("! mkdocs.yml not found — add extra_javascript/css manually:")
    print("  extra_javascript: [ docs/assets/js/version_badge.js ]")
    print("  extra_css:        [ docs/assets/css/version_badge.css ]")
    return
  text = mk.read_text(encoding="utf-8")

  # Ensure extra_javascript includes our file
  ...
  # Ensure extra_css includes our file
  ...
  print("→ patched mkdocs.yml (extra_javascript + extra_css)")

def main():
  js = JS.replace("%OWNER%", OWNER).replace("%REPO%", REPO)
  write(ROOT / "docs" / "assets" / "js" / "version_badge.js", js)
  write(ROOT / "docs" / "assets" / "css" / "version_badge.css", CSS)
  patch_mkdocs()
  print("\n✅ Version badge installed.")
  print("Next: commit/push and your docs will show the latest tag in a floating badge.")
