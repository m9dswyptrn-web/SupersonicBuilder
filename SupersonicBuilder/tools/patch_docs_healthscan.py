...
BLOCK = dedent("""
    ...
    ## 🧼 Supersonic Hygiene & Release Routine — v4.0.1 and Later
    ...
    ### 🔧 One-Shot Local Hygiene Commands
    ```bash
    make hooks lint typecheck
    make health-scan
    make health-open
    ```
    - **hooks** → runs all pre-commit checks  
    - **lint** → applies Ruff + Black auto-fixes  
    - **typecheck** → runs MyPy validation  
    - **health-scan** → performs repository-wide structural scan  
    - **health-open** → opens the generated report (`docs/HEALTH_REPORT.md`)

    ---

    ### 🚀 Hygiene Release Commit Flow
    ```bash
    git add -A
    ...
    ### 🧮 Optional — Automated Release via CLI
    ```bash
    ...
    **Result:**  
    ...
    |-------|----------|----------------|
    | 🟩 Lint Clean | `make lint` | "0 violations" |
    | 🟩 Type Safe | `make typecheck` | "Success: no issues" |
    | 🟩 Health OK | `make health-scan` | "No missing integrations" |
    | 🟩 Docs Up-to-date | `make release-budgets` | new `docs/budgets.html` and SVG |
...
def main():
    ...
    if not args.no_tag:
        ...
        run("git push")
        run("git push --tags")
        print(f"✅ Pushed tag {args.version}")
    else:
        run("git push")
        print("✅ Changes pushed (no tag)")
