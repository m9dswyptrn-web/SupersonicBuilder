...
def patch_server():
    ...
    print("✅ Patched serve_pdfs.py with rotating file logger and /logs/tail")

def patch_panel():
    if not TPL.exists():
        print("⚠️ templates/panel.html not found; skipping UI patch.")
        return
    backup(TPL)
    html = TPL.read_text(encoding="utf-8")
    if "btnLogs" in html and "/logs/tail" in html:
        print("✅ Panel already has Logs button.")
        return

    html = html.replace(
        'id="btnHealth"',
        'id="btnHealth" class="btn" style="background:#2ea043;">❤️ Health</button>\n  '
        '<button id="btnLogs" class="btn" style="background:#6e7681;">🧾 Logs</button><button'
    ...
    print("✅ Panel updated with 🧾 Logs button")

if __name__ == "__main__":
    patch_server()
    patch_panel()
    print("\n🎉 Done. Reload your app and open /panel → click 🧾 Logs to view the last 400 lines.")
