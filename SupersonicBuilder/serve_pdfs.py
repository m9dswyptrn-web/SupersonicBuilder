...
from flask import Flask, jsonify, request, send_from_directory, Response, request, render_template_string, redirect, url_for, render_template
...
def _supersonic_req_start():
    request._supersonic_t0 = time.time()  # type: ignore[attr-defined]
  ...
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
      ...
      <span id="totalBox" class="small">
        ⬇️ total downloads: <strong id="totalCnt">0</strong>
      </span>
    </div>
    <table id="tbl">
    ...
    try{
      ...
      items.forEach(it=>{
        ...
        tr.innerHTML =
          ...
          `<td>${cnt}</td>`+
          `<td><a href="/downloads/${it.name}" download>⬇️</a></td>`;
      ...
      document.getElementById('foot').textContent =
        ...
        ' • Stats updated '+new Date((stats.updated||0)*1000).toISOString();
...
def favicon():
    """Handle favicon requests gracefully to avoid 500 errors."""
